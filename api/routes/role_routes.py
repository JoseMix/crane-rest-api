...
from api.config.constants import OPA_RBAC_CONFIG_NAME, OPA_RBAC_CONFIG_FILE
from api.routes.auth_routes import verify_jwt
...
def get_all_roles(skip: int = 0, limit: int = 100, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
    return get_all(db=db, skip=skip, limit=limit)


@roleRouter.get("/{role_id}", response_model=Role)
def get_role_by_id(role_id: int, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
...
def create_role(role: RoleCreate, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
    return create(db=db, role=role)


@roleRouter.put("/{role_id}", response_model=Role)
def update_role(role_id: int, role: RoleUpdate, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
...
def delete_role(role_id: int, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
...
def create_role_for_user(user_id: int, role_id: int, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
    return create_for_user(db, user_id, role_id)


@roleRouter.delete("/{role_id}/user/{user_id}")
def remove_role_from_user(user_id: int, role_id: int, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
    return remove_user(db, user_id, role_id)


@roleRouter.get("/user/{user_id}")
def get_user_roles(user_id: int, db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
    return get_roles_by_user(db, user_id)


@roleRouter.patch("/OPA/reload")
async def reload_opa(db_user=Depends(verify_jwt), db: Session = Depends(get_db)):
    update_policies_file(OPA_RBAC_CONFIG_NAME, OPA_RBAC_CONFIG_FILE, True)
    return {"message": "OPA policies reloaded"}
